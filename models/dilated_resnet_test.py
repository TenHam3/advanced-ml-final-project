...
class _MultiDilatedConv(nn.Module):
    """
    Shared-weight multi-dilation conv — the conv operation from MultiDilatedConv
    without the internal skip connection. Intended to be used inside BasicBlock,
    ...
    def forward(self, x):
        outputs = [
            F.conv2d(x, self.conv.weight, None,
                     padding=d * (self.kernel_size // 2), dilation=d)
...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, dilations=(1, 2, 4)):
        super().__init__()
        self.conv1 = _MultiDilatedConv(in_channels, out_channels, kernel_size=3, dilations=dilations)
        ...
        self.conv2 = _MultiDilatedConv(out_channels, out_channels, kernel_size=3, dilations=dilations)
...
class DilatedResNetTest(nn.Module):
    """
    ResNet-18 with every 3x3 conv replaced by a shared-weight MultiDilatedConv.
